...
def plot_borda_ranking(borda_ranking, agg_ranking, title="Borda Color Coded by True"):
    """
    Plot a bar chart of items and their scores, sorted high→low, 
    with colors ranging from green (high) to red (low).
    ...
    Args:
        ...
        title (str): Plot title.
    """
    items = np.array(list(borda_ranking.keys()))
    scores = np.array(list(borda_ranking.values()))
    ...
    # Normalize scores for color mapping
    agg_scores = np.array([agg_ranking[item] for item in items])
    norm = (agg_scores - agg_scores.min()) / (agg_scores.max() - agg_scores.min() + 1e-9)
    cmap = plt.cm.get_cmap('RdYlGn')
    colors = [cmap(v) for v in norm]
